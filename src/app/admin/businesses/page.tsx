    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center border-b border-slate-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Audit profile registries, payment ledger volume, subscription tiers, and features.</p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => setShowCreateModal(true)}
            className="flex items-center gap-1.5 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-xs font-semibold text-white transition-all shadow-sm shadow-indigo-650/10"
          >
          ...
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 px-3 py-2 border border-slate-800 rounded-lg hover:bg-slate-800/40 text-xs font-semibold text-slate-300 hover:text-white transition-all"
          >
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-[#1E293B]/30 border border-slate-800/80 rounded-xl p-4 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs font-medium text-slate-300 focus:outline-none focus:border-slate-700"
          >
        ...
        <button
          ...
          className="bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg px-4 py-2 text-xs font-semibold shadow-sm transition-all"
        >
          Apply Filters
        </button>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/50 text-xs font-medium text-slate-300">
              {loading ? (
                ...
                businesses.map((biz) => {
                  ...
                  return (
                    <tr key={biz.id} className="hover:bg-slate-800/20 transition-all">
                      {/* Name & Owner info */}
                      <td className="p-4">
                        <div>
                          <p className="font-semibold text-white text-[13px]">{biz.name}</p>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wide border ${
                            planName === "free" ? "bg-slate-800/40 text-slate-400 border-slate-700/30" :
                            planName === "solo" ? "bg-indigo-500/10 text-indigo-400 border-indigo-500/20" :
                            planName === "business" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                            "bg-amber-500/10 text-amber-400 border-amber-500/20"
                          }`}>
                            {planName}
                          </span>
                          <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                            subStatus === "active" || subStatus === "trialing" ? "bg-emerald-500" : "bg-slate-600"
                          }`} />
                        </div>
                      </td>
                      ...
                      <td className="p-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Link
                            ...
                            className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 border border-slate-700/60 rounded-lg text-xs font-semibold text-white transition-all shadow-sm"
                          >
                            Manage
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center p-2 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 hover:border-rose-500/30 rounded-lg text-rose-400 transition-all"
                          >
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#1E293B] border border-slate-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            {/* Modal Header */}
            <div className="p-5 border-b border-slate-800/80 flex justify-between items-center bg-slate-900/40">
              <div>
                <h3 className="text-sm font-bold text-white">Create New Business Profile</h3>
            ...
            <form onSubmit={handleCreateBusiness} className="p-5 space-y-4">
              ...
              <div className="pt-2 flex gap-3">
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="w-1/2 border border-slate-800 hover:bg-slate-800 text-slate-400 hover:text-white rounded-lg py-2.5 text-xs font-semibold transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={creating}
                  className="w-1/2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg py-2.5 text-xs font-semibold shadow-sm transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
                  {creating ? (
                    <>
