...
function StatCard({ title, value, subText, icon, colorClass }: StatCardProps) {
  return (
    <div className="bg-[#1E293B]/40 border border-slate-800/80 rounded-xl p-5 shadow-sm hover:border-slate-700/60 transition-all duration-200">
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center border-b border-slate-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Real-time system health metrics, plan allocations, and activity telemetry.</p>
        </div>
        <button
          onClick={() => fetchStats(true)}
          disabled={isRefreshing}
          className="p-2 border border-slate-800 rounded-lg hover:bg-slate-800/40 text-slate-400 hover:text-white transition-all disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-[#1E293B]/30 border border-slate-800/80 rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="text-sm font-semibold text-white tracking-tight border-b border-slate-800 pb-3 flex items-center justify-between">
              <span>SaaS Subscription Distribution</span>
              <span className="text-[10px] text-indigo-400 uppercase tracking-widest font-bold">Plan counts</span>
            </h3>
            <div className="space-y-4 pt-1">
              {[
                { name: "Free Tier", count: freeCount, color: "bg-indigo-500", pct: `${Math.round((freeCount / totalSubs) * 100)}%` },
                { name: "Solo Tier", count: soloCount, color: "bg-emerald-500", pct: `${Math.round((soloCount / totalSubs) * 100)}%` },
                { name: "Business Tier", count: businessCount, color: "bg-amber-500", pct: `${Math.round((businessCount / totalSubs) * 100)}%` },
                { name: "Scale Tier", count: scaleCount, color: "bg-rose-500", pct: `${Math.round((scaleCount / totalSubs) * 100)}%` },
              ].map((plan, idx) => (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between items-center text-xs font-medium text-slate-400">
                    <span>{plan.name}</span>
                    <span className="text-white font-semibold">{plan.count} ({plan.pct})</span>
                  </div>
          ...
          <div className="bg-[#1E293B]/30 border border-slate-800/80 rounded-xl p-5 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-sm font-semibold text-white tracking-tight">Recent Onboardings</h3>
              <Link href="/admin/businesses" className="text-[10px] text-indigo-400 font-bold hover:underline flex items-center gap-1 uppercase">
            ...
            <div className="space-y-3 pt-1">
              {data?.recentBusinesses?.length === 0 ? (
                <p className="text-xs text-slate-500 italic py-3 text-center">No client registrations logged in this snapshot.</p>
              ) : (
                data?.recentBusinesses?.map((biz: any) => (
                  <div key={biz.id} className="flex justify-between items-center border-b border-slate-800/30 pb-2 last:border-b-0">
                    <div>
                    ...
                    <span className="text-[9px] text-slate-400 bg-slate-800/40 border border-slate-700/30 px-1.5 py-0.5 rounded font-mono">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Activity feed */}
          <div className="bg-[#1E293B]/30 border border-slate-800/80 rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="text-sm font-semibold text-white tracking-tight border-b border-slate-800 pb-3 flex items-center gap-2">
            ...
            <div className="space-y-4 pt-1 max-h-[380px] overflow-y-auto pr-1">
              {data?.recentInvoices?.length === 0 ? (
                ...
                data?.recentInvoices?.map((inv: any) => (
                  <div key={inv.id} className="flex items-start gap-3 border-l-2 border-slate-800 pl-4 py-0.5 position-relative">
                    <div className="flex-1">
                      <p className="text-xs font-semibold text-white">
                        Invoice <span className="font-mono text-indigo-400">{inv.invoice_number}</span> created
                      </p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[8px] text-slate-500 font-mono mt-1">{formatDate(inv.created_at)}</p>
                    </div>
                  </div>
                ))
              )}
