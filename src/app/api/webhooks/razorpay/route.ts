...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: webhookLog, error: logErr } = await adminDb
    ...
    const amountPaidInPaise = eventEntity.amount || Math.round(invoice.balance_due * 100)
    const amountPaidInRupees = amountPaidInPaise / 100

    const newAmountPaid = (invoice.amount_paid || 0) + amountPaidInRupees
    ...
    const { data: payment } = await adminDb
      ...
      .insert({
        ...
        payment_method: eventEntity.method || "upi",
    ...
    if (invoice.client_id) {
      ...
      if (client) {
        await adminDb
          .from("clients")
          .update({
            total_paid: (client.total_paid || 0) + amountPaidInRupees,
            updated_at: nowIso
          })
          .eq("id", invoice.client_id)
      }
    ...
    // 7. Create Workspace Notification
    ...
    return NextResponse.json({ success: true, invoice_id: invoiceId, status: isFullyPaid ? "paid" : "partial" })
  } catch (err: any) {
